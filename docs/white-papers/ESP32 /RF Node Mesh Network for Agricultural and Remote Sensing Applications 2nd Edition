...
## 11. Autonomous Ecological Restoration Mode (AERM)

AERM enables autonomous aerial rewilding missions using biodegradable seed balls embedded with native species. The drone can:

* **Deploy Seed Balls** via servo-based or rotary gravity-fed dispenser
* **Navigate Regeneration Zones** mapped via onboard GPS or internal mesh-coordinated coordinates
* **Time Deployments Seasonally** using:

  * Rainfall forecasts from base station
  * Soil moisture trends from ground nodes
  * Temperature data from the drone’s own sensor stack
* **Avoid Detection** by:

  * Flying below radar-significant altitudes
  * Using private ESP32/LoRa GPS-less location matrix
  * Operating in early morning/evening when air is stable

**Seed Payloads**: Native wattles, sheoaks, saltbush, or other pioneer plants with strong soil-binding roots and high drought resilience. Embedded with clay/char/soil for germination buffering.

**Drop Pattern Algorithms**:

* Clustered drop for wind-buffering groves
* Linear drop for erosion gully recovery
* Random walk drop for mimicking natural scatter
